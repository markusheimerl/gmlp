...
int main() {
    ...
    printf("Starting training GPU gMLP...\n");
    ...
    strftime(model_fname, sizeof(model_fname), "%Y%m%d_%H%M%S_gmlp_gpu_model.bin", 
             localtime(&now));
    strftime(data_fname, sizeof(data_fname), "%Y%m%d_%H%M%S_data_gpu.csv", 
             localtime(&now));
    ...
    for (int i = 0; i < output_dim; i++) {
        ...
        float mae = 0.0f;
        for (int j = 0; j < num_samples; j++) {
            mae += fabsf(loaded_gmlp->predictions[j * output_dim + i] - y[j * output_dim + i]);
        }
        mae /= num_samples;
